...
class ReceiptPrinter
{
    ...
    public function printReceipt($with_items = true) {
        if ($this->printer) {
            ...
            $dp = $this->getPrintableSummary('DP / Diskon', $this->dp);
            $grandtotal = $this->getPrintableSummary('Total Bayar', $this->subtotal - $this->dp);
            ...
            if ($this->dp != 0) {
                $this->printer->text("\n");
                $this->printer->text($dp);
                $this->printer->text("\n");
                $this->printer->setEmphasis(false);
                $this->printDashedLine();
                $this->printer->text("\n");
                $this->printer->setEmphasis(true);
                $this->printer->text($grandtotal);
                $this->printer->feed();
            } else {
                $this->printer->feed();
            }
